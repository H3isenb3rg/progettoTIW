...
public class GoToAuctionDetails extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String path = "/WEB-INF/AuctionDetail.html";
		...
		try {
			...
			response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Not possible to recover auctions and bids");
			return;
		}
		ServletContext servletContext = getServletContext();
		...
		if(auction.isClosed()) {
			if(bids.size()<=0) 
				ctx.setVariable("receivedOffers", false);
			else {
				ctx.setVariable("receivedOffers", true);
				UserDAO userDAO = new UserDAO(this.connection);
				Bid winnerBid = bids.get(0);
				ctx.setVariable("winner", winnerBid.getBidder());
				ctx.setVariable("price", winnerBid.getOffer());
				User winner;
				try {
					winner = (User) userDAO.getUserByUsername(winnerBid.getBidder());
				} catch (SQLException e) {
					e.printStackTrace();
					response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Not possible to recover winner");
					return;
				}
				ctx.setVariable("address", winner.getAddress());
			}
		}
